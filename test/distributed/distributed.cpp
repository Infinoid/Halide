...
int main(int argc, char **argv) {
...

    {
        ...
        int num_elements = 16383;
        Buffer<int> in_buf(nullptr, 0, 0);
        in_buf.set_distributed(std::vector<int>{2, num_elements});
        f.infer_input_bounds({2, num_elements}, get_jit_target_from_environment(), {{in, &in_buf}});
        in.set(in_buf);
        int num_elements_per_proc = (num_elements + numprocs - 1) / numprocs;
        int buf_min = num_elements_per_proc * rank;
        int buf_max = std::min(buf_min + num_elements_per_proc - 1, num_elements - 1);
        ...
        for (int x = out.dim(0).min(); x <= out.dim(0).max(); x++) {
            int correct = 4 * x + 1;
            if (out(0, x) != correct) {
                printf("out(0, %d) = %d instead of %d\n", x, out(0, x), correct);
                MPI_Finalize();
                return -1;
            }
            if (out(1, x) != correct) {
                printf("out(1, %d) = %d instead of %d\n", x, out(1, x), correct);
                MPI_Finalize();
                return -1;
            }
        }
